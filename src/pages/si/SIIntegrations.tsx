...
function StatusBadge({ connected, badge }: { connected: boolean; badge?: "Recommended" }) {
  ...
  if (badge === "Recommended") {
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold bg-indigo-50 text-indigo-600 border border-indigo-200 whitespace-nowrap">
        Recommended
      </span>
    );
  }
  ...
  return (
    ...
    >
      ...
      {disconnectConfirm ? (
        ...
        <Button
          size="sm"
          className="w-full h-9 text-sm bg-indigo-600 hover:bg-indigo-700 text-white transition-colors duration-200"
          onClick={onConnect}
        >
          Connect
        </Button>
  ...
  return (
    <div
      className={cn(
        ...
        isZero ? "bg-indigo-50 border-indigo-200" : "bg-blue-50 border-blue-200"
      )}
    >
      <div className="flex items-start gap-3">
        <Icon
          icon={isZero ? "solar:danger-circle-linear" : "solar:info-circle-linear"}
          className={cn("h-5 w-5 mt-0.5 flex-shrink-0", isZero ? "text-indigo-600" : "text-blue-600")}
        />
        <p className={cn("text-sm font-medium", isZero ? "text-indigo-700" : "text-blue-700")}>
          {isZero
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-10">
          ...
          <div className="lg:sticky lg:top-8 self-start space-y-8">
            ...
            <button className="flex items-center gap-2 text-sm text-gray-400 hover:text-indigo-600 transition-colors duration-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {filteredIntegrations.length > 0 ? (
                ...
                <div className="col-span-2 flex flex-col items-center justify-center py-16 text-center">
                  <Icon icon="solar:widget-5-linear" className="h-10 w-10 text-gray-300 mb-3" />
                  <p className="text-sm text-gray-400">No integrations in this category yet.</p>
                </div>
              )}
            </div>
            ...
            {activeCategory === "All" && (
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <Icon icon="solar:clock-circle-linear" className="h-4 w-4 text-gray-400" />
